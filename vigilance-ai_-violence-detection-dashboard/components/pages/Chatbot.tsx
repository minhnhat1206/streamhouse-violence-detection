...
const Chatbot: React.FC = () => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const { answer, layer, citations } = await sendChatMessage(
        input,
        chatHistory.map((m) => ({ role: m.role, content: m.content })),
      );
      const modelMessage: ChatMessage = { role: 'model', content: answer, layer, citations };
      setMessages((prev) => [...prev, modelMessage]);
    } catch (err) {
      const modelMessage: ChatMessage = {
        role: 'model',
        content: 'Lỗi kết nối backend. Vui lòng kiểm tra chatbot service (port 5002).',
      };
      setMessages((prev) => [...prev, modelMessage]);
    }
    setIsLoading(false);
              ...
              <div className={`px-4 py-3 rounded-xl max-w-lg ${
                ...
                {message.role === 'model' && message.citations && (
                  <p className="text-xs text-slate-500 mt-2 italic">
                    Layer: {message.layer} · {message.citations.source_table} · {message.citations.time_period}
                  </p>
                )}
              </div>
               {message.role === 'user' && (
