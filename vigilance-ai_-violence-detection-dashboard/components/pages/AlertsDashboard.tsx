...
import { fetchRecentIncidents } from '../../services/backendService';
import { ChevronDown, ChevronUp, Video, X, RefreshCw, AlertCircle } from 'lucide-react';
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50">
        <div className="relative bg-slate-900 rounded-xl shadow-2xl w-full max-w-2xl border border-slate-700">
             <div className="p-4 flex justify-between items-center border-b border-slate-800">
                ...
                <button onClick={onClose} className="p-2 rounded-full hover:bg-slate-800"><X size={20} /></button>
            </div>
            <div className="p-6">
                <div className="aspect-video bg-black rounded-lg mb-4 flex items-center justify-center">
                    <Video className="w-16 h-16 text-slate-600"/>
                    <p className="absolute text-white">Video evidence clip player</p>
                </div>
...
const AlertsDashboard: React.FC = () => {
  ...
  const [sortConfig, setSortConfig] = useState<{ key: keyof Alert; direction: 'asc' | 'desc' } | null>({ key: 'timestamp', direction: 'desc' });
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const data = await fetchRecentIncidents(50);
        if (!cancelled) {
          setAlerts(data);
          setError(null);
        }
        ...
        if (!cancelled) {
          ...
          setError('Backend không khả dụng — đang hiển thị dữ liệu mẫu.');
        }
      } finally {
    ...
    <div className="bg-slate-900/50 rounded-xl p-6 border border-slate-800">
      {loading && (
        <div className="flex items-center gap-2 text-slate-400 mb-4 text-sm">
          ...
          Đang tải dữ liệu từ Streamhouse...
        </div>
      )}
      {error && (
        <div className="flex items-center gap-2 text-yellow-400 bg-yellow-500/10 border border-yellow-500/30 rounded-lg px-4 py-2 mb-4 text-sm">
          <AlertCircle size={14} />
          {error}
        </div>
      )}
